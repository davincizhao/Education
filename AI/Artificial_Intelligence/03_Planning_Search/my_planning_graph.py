...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for act in self.all_actions:
            a_Node = PgNode_a(act)
            # if a proposed PgNode_a has prenodes that are asubset of the previous S level.
            if a_Node.prenodes <= self.s_levels[level]:
                
                for  s in self.s_levels[level]:
                   
                    for a_p in a_Node.prenodes:
                        #use prenode of A node to determine the parent node of A Node in S node 
                        if s == a_p:
                            a_Node.parents = s
                            s.children = a_Node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (node_a1.action.effect_add == node_a2.action.effect_rem 
            or node_a1.action.effect_rem == node_a2.action.effect_add):
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if ((node_a1.action.effect_add == node_a2.action.precond_neg 
            and node_a1.action.effect_add !=[])
            or (node_a1.action.effect_rem == node_a2.action.precond_pos
            and node_a1.action.effect_rem !=[])
            or (node_a2.action.effect_add == node_a1.action.precond_neg
            and node_a2.action.effect_add !=[])
            or (node_a2.action.effect_rem == node_a1.action.precond_pos
            and node_a2.action.effect_rem !=[])):
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #if pa1.is_mutex(pa2) :
        #print("type",pa1.symbol,pa2.symbol)
        if ((node_a1.action.precond_pos == node_a2.action.precond_neg 
            and node_a1.action.precond_pos !=[])
            or (node_a1.action.precond_neg == node_a2.action.precond_pos
            and node_a1.action.precond_neg !=[])
            ):
            return True
        elif (isinstance(pa1, set) and isinstance(pa2, set)):

            if list(pa1)[0].is_mutex(list(pa2)[0]):
                return True
            return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        if (isinstance(ps1, set) and isinstance(ps2, set)):

            if list(ps1)[0].is_mutex(list(ps2)[0]) and len(list(ps1)) ==1 and len(list(ps2)) == 1:
                return True
        else:
            if ps1.is_mutex(ps2) :
                return True
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for s_level in self.s_levels:
                number_l +=1
                for s in s_level:
                    if g == s.symbol and s not in ved_st:
                        level_sum += number_l
                        ved_st.add(s)
                        break
